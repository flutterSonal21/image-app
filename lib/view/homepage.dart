...
Widget _buildImageCarousel() {
  ...
  final totalPages = (totalImages / imagesPerPage).ceil(); // Calculate total pages
  ...
  return Obx(() {
    ...
    return Stack(
      children: [
        SizedBox(
          ...
          child: PageView.builder(
            ...
            onPageChanged: (index) async {
              ...
              // Load more images when reaching the last page
              if (index == totalPages - 1 && !homeController.isLoadingNewImages.value) {
                await homeController.loadMoreImages();
              }
            },
            itemBuilder: (context, pageIndex) {
              // Correct start and end index for the current page
              int start = pageIndex * imagesPerPage;
              int end = start + imagesPerPage;

              // Ensure the end index does not exceed the available images
              if (end > limitedPhotos.length) {
                end = limitedPhotos.length;
              }

              // Slice the images for the current page
              List currentImages = limitedPhotos.sublist(start, end);
                  ...
                  return GestureDetector(
                    onTap: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => DetailsView(index: start + index),
                        ),
                      );
                    },
                    child: Container(
                      ...
                      child: CachedNetworkImage(
                        imageUrl: currentImages[index].urls!['small']!,
        ...
        // Circle page indicators for pagination
        // Circle page indicators for pagination
Positioned(
  bottom: 20,
  left: 0,
  ...
  child: Center(
    child: Obx(() {
      // Calculate the current page based on the current image index
      int currentPageIndex = currentPage.value;
      ...
      return Row(
        ...
        children: List<Widget>.generate(totalPages, (index) {
          return GestureDetector(
            onTap: () {
              // Move to the specific page when the circle is tapped
              pageController.animateToPage(
                index,
                duration: Duration(milliseconds: 500),
                curve: Curves.easeInOut,
              );
              currentPage.value = index; // Update current page when tapped
            },
            child: Padding(
